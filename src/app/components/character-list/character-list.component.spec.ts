...
describe('CharacterListComponent', () => {
  ...
  beforeEach(() => {
    ...
    character = mockObject;
  });

  it('deve alternar o status de favorito e chamar o método de serviço correto', () => {
    component.favorite(character);
    expect(character.favorite).toBe(true);
    expect(charactersService.addFavorite).toHaveBeenCalledWith(character);
    component.favorite(character);
    expect(character.favorite).toBe(false);
    expect(charactersService.removeFavorite).toHaveBeenCalledWith(character);
  });
});
